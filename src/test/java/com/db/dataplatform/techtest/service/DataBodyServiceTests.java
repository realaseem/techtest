...
public class DataBodyServiceTests {
    ...
    public void shouldSaveDataBodyEntityAsExpected(){
        ...
        verify(dataStoreRepositoryMock).save(expectedDataBodyEntity);
    }

    @Test
    public void shouldGetDataBodyEntitiesByTypeAsExpected() {
        BlockTypeEnum blockTypeEnum = BlockTypeEnum.BLOCKTYPEA;

        dataBodyService.getDataByBlockType(blockTypeEnum);

        verify(dataStoreRepositoryMock).findDataBodyEntitiesByDataHeaderEntityBlocktype(blockTypeEnum);
    }

    @Test
    public void shouldGetDataBodyEntitiesByNameAsExpected() {
        String name = "someBlockName";

        dataBodyService.getDataByBlockName(name);

        verify(dataStoreRepositoryMock).findDataBodyEntityByDataHeaderEntityName(name);
    }
